...
import {
  AppBar, Divider, Toolbar, Typography, Input, Fab, IconButton,
...
function MessageChat({ user, contactList }) {
      ...
      if (msg.message_type === 'text') { // text
        const deletebutton = (
          <div id="converDiv">
            ...
            <DeleteMessageButtonComponent userid={userId} contactid={contact} messageContent={msg.content} />
          </div>
        );
  ...
  useInterval(() => {
    const fetchMessages = async () => {
      ...
      console.log('message component conversation id', conversationId);
  ...
  useEffect(() => {
    ...
    const async_cb2 = async () => {
      try {
        ...
        if (!conversationId || conversationId.length === 0) { // create conversation + persist sid
          console.log('create new conversation id in front end message comp');
          const new_sid = await createConversation(contact, contactCID);
          console.log('created conversation id', new_sid);
