...
router.get('/contacts', auth, async (req, res) => {
  ...
  const contactList = await Contact.aggregate([
    {
      $match: {
        $or:
        [{
          user1: ObjectId("5fb35c87f88f9c9d1fca97fd")
        }, {
          user2: ObjectId("5fb35c87f88f9c9d1fca97fd")
        }]
      }
    },
    {
  ...
  const newList = contactList.map((contactObj) => {
    ...
    const nickname = (contactObj.user1 === user) ? contactObj.fromUser1[0].username : contactObj.fromUser2[0].username;
    const finalObj = {
      username: nickname,
      id: userContact,
    };
