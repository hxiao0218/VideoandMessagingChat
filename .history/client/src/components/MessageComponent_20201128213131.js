...
function MessageChat({ user, contactList }) {
  ...
  const validateFile = (curType) => {
    console.log('got to validate file!');
    const curSize = ((file.size / 1024) / 1024).toFixed(4); // MB
    const curName = file.name;
    const validateName = curName.match(validObj[curType]);
    console.log('size', curSize, 'validate name', validateName);
