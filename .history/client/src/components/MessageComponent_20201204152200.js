...
function MessageChat({ user, contactList }) {
  ...
  useInterval(() => {
    const fetchMessages = async () => {
      ...
      // TODO: render media list!
      setMediaList(mediaResp);
    };
    fetchMessages();
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (!conversationId) { // create conversation + persist sid
        const new_sid = createConversation(contact, contactCID);
        console.log(new_sid);
        setConversationId(new_sid);
      } else { // fetch conversation
        const resp = fetchConversation(conversationId);
        console.log(resp);
      }
    } catch (error) {
      console.log(error);
  ...
  const uploadPhoto = async (e) => {
    ...
    if (!validateFile(curFile)) {
      ...
      try {
        ...
        //   const mediaSID = await twilioMediaUpload(imageBlob);
        //   console.log('photo link', mediaSID);
        //   sendTwilioMessage(userId, mediaSID, conversationId);
        // };
        // fileReader.readAsArrayBuffer(curFile);
        fileReader.onload = (fe) => {
          const async_cb = async () => {
            const binFile = fe.target.result;
            const imageType = 'image/png';
            console.log('binFile', binFile);
            const mediaSID = await twilioMediaUpload(binFile, imageType);
            console.log('photo link', mediaSID);
            await sendTwilioMessage(userId, mediaSID, conversationId);
          };
          async_cb();
        };
