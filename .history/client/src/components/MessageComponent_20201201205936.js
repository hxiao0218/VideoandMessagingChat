...
import { getMessages, sendMessage, retrieveToken } from '../network/getData';
...
function MessageChat({ user, contactList }) {
  ...
  console.log('curContact', curContact, contactName, contactSID);
  ...
  useEffect(() => {
    if (!conversationsClient.on) return;
    console.log('got ehre!', conversationsClient);
    ...
    try {
      if (!contactSID) { // create conversation + persist sid
        conversationsClient.conversations.conversations
          .create({
            messagingServiceSid: '',
            uniqueName: contact,
          })
          .then((conversation) => console.log('CONVERSATION CREATED: ', conversation.sid));
