...
export const twilioMediaUpload = async (file) => {
  const reader = new FileReader();
  reader.onload = (e) => {
    console.log(e.target.result);
  };
  reader.onerror = (e) => console.log('[uplaod read err]: ', e.type);
  const binFile = reader.readAsBinaryString(file);
  const res = await axios.post('https://mcs.us1.twilio.com/v1/Services/IS960c89f737814fd7baa53c4cf10a34b8/Media', null, {
    headers: { 'Content-Type': 'multipart/form-data' },
    data: binFile,
  });
